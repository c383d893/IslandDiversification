# This script:
# 1. generates counts of native species by myc type
...
# bring in myc dat family (native GIFT)
mdat.f <- readRDS("data/gift_myc_families.RDS") %>%                       
  rename(myc.f = myc)  
...
species <- species.dat[[2]] %>% as.data.frame() %>% 
  ...
  left_join(mdat.f, by = c("family"))  %>% 
  mutate(myc = coalesce(myc.s, myc.g, myc.f)) %>% 
  drop_na(myc) %>%                                                          
...
# what prop of each family belongs to each type? 
# assign >90 to one type
...
#saveRDS(familyprop, "data/gift_myc_families.RDS")
...
# start from species.dat to include removed data (non myc assignable)
...
saveRDS(dat, "data/native_myc_data_2025.RDS")
