# This script:
# 1. generates counts of non-endemic species by dispersal syndrome
...
# bring in disp dat species
ddat.s <- readRDS("data/dispersal_species_WCVP.RDS") %>%
  rename(disp.s = dispersal)

# bring in disp dat family
ddat.f <- readRDS("data/dispersal_family_WCVP.RDS") %>%
  rename(disp.f = dispersal)
...
# with species checklist data, keep native, assign disp preferentially using species, then family
...
species <- species.dat[[2]] %>% as.data.frame() %>% 
  ...
  left_join(ddat.s, by = c("species")) %>% 
  left_join(ddat.f, by = c("family"))  %>%                                     
  mutate(disp = ifelse(is.na(disp.s), disp.f, disp.s)) %>%    
  mutate(disp_level = ifelse(!is.na(disp.s), "species", "family")) %>%
  drop_na(disp) %>%                                                          
  select(-c("disp.s", "disp.f")) %>%  
...
################################################
##### TOTAL SP RICHNESS AND JOIN WITH DISP #####
################################################

# start from species.dat to include removed data (non disp assignable)
...
# join disp sp richness, replace NAs and join with sprich, and geo
dat <- sprich %>% 
  left_join(sprich.biotic, by = "entity_ID") %>%
  left_join(sprich.abiotic, by = "entity_ID") %>%
  mutate_at(vars(c('disp.b','disp.ab')), ~replace(., is.na(.), 0)) %>%
  left_join(geo, by = "entity_ID") 
...
saveRDS(dat, "data/nonend_disp_data_2025.RDS")
