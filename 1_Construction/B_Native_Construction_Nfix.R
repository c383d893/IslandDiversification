# This script:
# 1. generates counts of native species by nfixing status
...
ndat <- read.csv("data/Werner_NFix_WVCP.csv", header = TRUE) %>%     
...
# subset to known nfix species: 188417
species.known.nfix <- species %>%
  drop_na(data_fixing) 

# subset to the other species: 1692989
...
# start form species.dat to get even removed data (not strictly necessary here - all assigned)
...
saveRDS(dat, "data/native_nfix_data_2025.RDS")
